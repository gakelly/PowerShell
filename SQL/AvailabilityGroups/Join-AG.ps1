...
.EXAMPLE
   Join-AG -AGDatabase "Arc_NQSC-ARC-01-PR02" -AGName "AG-ARC-01-PR02"
.EXAMPLE
...
if(!($target = Get-SqlDatabase -ServerInstance $Instance.Primary -Name $AGDatabase -ErrorAction SilentlyContinue)){
Write-Error "$AGDatabase does not exist on $($Instance.Primary)"
break;
}
if($target.RecoveryModel -eq 'Simple'){
Write-Output -Message "Setting database [$AGDatabase] on primary instance [$($Instance.Primary)] to FULL recovery"
$target | Set-DbaDbRecoveryModel -RecoveryModel Full -Confirm:$false
}
